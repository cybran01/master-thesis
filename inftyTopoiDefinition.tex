...
One can show that all objects of a locally presentable category must be compact, here meaning $\kappa$-compact (\cref{def:kappaCompactObject}) for some regular cardinal $\kappa$ (see \cite[Remark 1.30 (1)]{access_and_pres_cats}).
However all compact objects of $\Top$ in this sense are discrete spaces (see \cite[Examples 1.14 (6)]{access_and_pres_cats}).
...
\begin{definition}[Universality] 
    ...
    We say that \emph{$C$ has universal colimits for $K$-shaped diagrams} if for all natural transformations $\alpha\colon p\to q\in\Fun(K^{\rhd},C)_1$ such that $q$ is a colimit cone and $\alpha$ is cartesian, then $p$ is also a colimit cone.
    
    We say that \emph{$C$ has universal colimits} if $C$ has universal colimits for $K$-shaped diagrams for every small simplicial set $K$.
\end{definition}
\begin{definition}[Descent] 
    Let $C$ be an \inftycat/ that admits small colimits and pullbacks and let $K$ be a small simplicial set.
    We say that \emph{$C$ has descent for $K$-shaped diagrams} if for all natural transformations $\alpha\colon p\to q\in\Fun(K^{\rhd},C)_1$ such that $p,q$ are colimit cones and $\alpha|_K$ is cartesian, then $\alpha$ is also cartesian.
    
    We say that \emph{$C$ has descent} if $C$ has descent for $K$-shaped diagrams for every small simplicial set $K$.
\end{definition}
\begin{remark}
    We will now describe universality and descent explicitly for pushouts ($\Lambda^2_0$-shaped diagrams), because these will be important cases as \cref{lem:univColimIffUnivPoAndCoprod} and \cref{lem:descentIffDescentPoAndCoprod} will show.
    ...
    \begin{center}
        \begin{tikzcd} [sep = .5 cm]
            \alpha(0,0,0) \arrow [dr] \arrow [rr] \arrow [dd] & & \alpha(0,1,0) \arrow [dr] \arrow [dd] \\
            & \alpha(1,0,0) \arrow [rr, crossing over] \arrow [dd] & & \alpha(1,1,0) \arrow [dd] & \\
            \alpha(0,0,1) \arrow [dr] \arrow [rr] & & \alpha(0,1,1) \arrow [dr] \\
            & \alpha(1,0,1) \arrow [from=uu, crossing over] \arrow [rr] & & \alpha(1,1,1)
        \end{tikzcd}\;.
    \end{center}
...
\begin{prop}\label{prop:colimitDecompositionPushouts} 
    ...
    \end{itemize}
    Then let $C$ be a cocomplete \inftycat/ and $\Phi\colon Y\to C$ a functor.
    For all $i\in I$, let $\Phi_i$ denote the induced map $\Phi_i\colon X(i)\to Y\to C$.

    Then the assignment $i\mapsto\colim\limits_{x\in X(i)} \Phi_i(x)$ naturally extends to a functor $\N(I)\to C$ such that there is a canonical invertible map of the form 
    \begin{equation*}
...
\begin{lemma}\label{lem:spineInclIsWCE}
    We view the linearly ordered set $\Nat$ as a category via \cref{rmk:linOrderedAsCat}.
...
\begin{lemma}\label{lem:univColimIffUnivPoAndCoprod}
    ...
    \begin{proof}
        ...
        Let $\dim K=n+1$. 
        Then by the skeletal filtration we have
        \begin{equation*}
            K=K^{n+1}\cong K^n\bigcup\limits_{\coprod\limits_S \partial\Delta^{n+1}}\left(\coprod_S \Delta^{n+1}\right)
        \end{equation*}
        ...
        Let $p_i,q_i\colon X_i^{\rhd}\to C$ be the maps obtained by precomposing $p$ and $q$ with the respective map to $K$ from the skeletal filtration and taking the conepoint of $q_i$ to be the colimit, and the conepoint of $p_i$ to be the pullback 
        \begin{center}
            \begin{tikzcd} [sep = 4em]
        ...
        \begin{center}
            \begin{tikzcd} [sep = 4em]
                ...
                q_2(*_s) \arrow[r] & q_2(\infty)\simeq\coprod\limits_{s\in S} q_2(*_s)
            \end{tikzcd}
        \end{center}
        ...
        \end{center}
        is a pushout square.
        Furthermore, the natural transformations $\alpha$ and $\alpha_i$ induce a natural transformation $\beta\in\Fun\left(\left(\Lambda_0^2\right)^{\rhd},C\right)_1$ which we represent by a cube
        \begin{center}
            \begin{tikzcd} [sep = .5 cm]
            ...
            \end{tikzcd}
        \end{center}
        and by the pasting law this natural transformation is also cartesian.
        ...
        For $q|_K\colon K\to C$, let $F_q\colon L^{\rhd}\to C$ be the colimit diagram constructed as described above.
...
\begin{lemma}\label{lem:descentIffDescentPoAndCoprod}
    ...
    \begin{proof}
        ...
        For $i\in\set{0,\ldots,n+1}$ and $s\in S$ let $i_s\in \left(X_2\right)_0$ denote the $i$-th vertex of the $s$-th component in $X_2=\coprod\limits_{s\in S} \Delta^{n+1}$.
        ...
        \begin{center}
            \begin{tikzcd} [sep = 4em]
                p_2(i_s) \arrow[r] \arrow[d] & p_2((i+1)_s) \arrow[r] \arrow[d] & \coprod\limits_{s\in S} p_2((n+1)_s)\simeq p_2(\infty) \arrow[d] \\
                q_2(i_s) \arrow[r] & q_2((i+1)_s) \arrow[r] & \coprod\limits_{s\in S} q_2((n+1)_s)\simeq q_2(\infty)
            \end{tikzcd}
        \end{center}
        ...
        For all objects $x\in \left(X_0\right)_0$ and $i\in\set{1,2}$ the left square of the diagram 
        \begin{center}
            \begin{tikzcd} [sep = 4em]
        ...
        The natural transformation $\alpha$ together with the $\alpha_i$ induces a natural transformation $\beta\in\Fun\left(\left(\Lambda_0^2\right)^{\rhd}, C\right)_1$ which we represent by a cube
        \begin{center}
            \begin{tikzcd} [sep = .5 cm]
                p_0(\infty) \arrow [dr] \arrow [rr] \arrow [dd] & & p_1(\infty) \arrow [dr] \arrow [dd] \\
                & p_2(\infty) \arrow [rr, crossing over] \arrow [dd] & & p(\infty) \arrow [dd] & \\
        ...
        We now show that $\alpha$ is cartesian:
        ...
        \begin{center}
            ...
            \end{tikzcd}
        \end{center}
        is a pullback by the pasting law since the left square is cartesian as $\alpha_i$ is cartesian and the right square is cartesian because $\beta$ is cartesian.
        ...
        \begin{center}
            ...
            \end{tikzcd}
        \end{center}
        is a pullback and hence by the pasting law so is the left square.
        ...
        \begin{center}
            ...
            \end{tikzcd}\;.
        \end{center}
        Since we have already proven that both left and right square of this diagram are cartesian, the outer square is also cartesian by the pasting law which proves the proposition.
    \end{proof}
\end{lemma}