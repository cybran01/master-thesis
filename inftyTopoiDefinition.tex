...
\begin{prop}\label{prop:colimitDecompositionPushouts} %TODO CatLR Proposition 7.3.16 (applied to C^{\op}) gives more general condintions, Remark 6.2.7 for we preseve colim, Proposition 3.7.4 CatLR + Corollary 3.9.8 CatLR for wce and L\subset\N(\Nat)
    ...
    For $i\in\set{0,1,2}$, set $X_i$ to be the colimit of $K_i\to K\to C$ in $C$.
    Then the induced diagram $\overline{p}\colon K^{\rhd}\to C$ given by $\overline{p}|_K=p$ and universal maps to the conepoint $\overline{p}(\infty)=X_1\cup_{X_0}X_2$ is a colimit cone in $C$.
    \begin{reference}
...
\begin{lemma}\label{lem:univColimIffUnivPoAndCoprod}
    ...
    \begin{proof}
        ...
        Then we have $q_2(\infty)\cong\coprod\limits_{s\in S} q_2(*_s)$. %TODO cref to amended prop
        ...
        Now by \cref{prop:colimitDecompositionPushouts} we know that 
        \begin{center}
            \begin{tikzpicture}
        ...
        Let $L\subset\N(\Nat)$ (where we view $\Nat$ as a category via the ordering $0\to1\to2\to3\ldots$) be the simplicial subset containing all $0$-simplices and only the $1$-simplices which are between consecutive integers (with all higher simplices degenerate). %TODO use def of partially ordered
        ...
        Since $L\subset\N(\Nat)$ is a weak categorical equivalence (which preserve and reflect (co)limits), we can also compute the colimit of the induced diagram $L\to C$.
        ...
        Universality for finite dimensional diagrams also implies that $F_p(n)$ is the colimit of $p|_{K^n}$ for every $n\in\Nat$, hence $F_p(\infty)$ is a colimit of $p|_K$. %TODO cite
...
\begin{lemma}\label{lem:descentIffDescentPoAndCoprod}
    ...
    \begin{proof}
        ...
        Since the objects $(n+1)_s$ are terminal in their component $\Delta^{n+1}$, we have $q_2(\infty)\cong\coprod\limits_{s\in S} q_2((n+1)_s)$ as well as $p_2(\infty)\cong\coprod\limits_{s\in S} p_2((n+1)_s)$. %TODO cite
        ...
        From this we conclude that the natural transformation $\beta\in\Fun\left(\left(\Lambda_0^2\right)^{\rhd}, C\right)_1$ induced by $\alpha$ and the $\alpha_i$ which we again represent by a cube
        \begin{center}
            \begin{tikzcd} [sep = .5 cm]
                p_0(\infty) \arrow [dr] \arrow [rr] \arrow [dd] & & p_1(\infty) \arrow [dr] \arrow [dd] \\
                & p_2(\infty) \arrow [rr, crossing over] \arrow [dd] & & p(\infty) \arrow [dd] & \\
        ...
        Furthermore, let $L\subset\N(\Nat)$ be defined as in the previous lemma.
        Let $K^n\subset K$ denote the $n$-th skeleton of $K$ and let $F_p\colon L^{\rhd}\to C$ an $F_q\colon L^{\rhd}\to C$ be functors that 
        \begin{itemize}
        ...
        \end{itemize}
        so in particular, they are colimit cones. %TODO cite
        ...
        But every such $x$ is already in some $K^n$, so we obtain a factorization
        \begin{center}
            \begin{tikzcd} [sep = 4em]
            ...
            \end{tikzcd}\;.
        \end{center}
        But we have already proven that both left and right square of this diagram are cartesian, hence the outer square is also cartesian which proves the proposition.
    \end{proof}
\end{lemma}