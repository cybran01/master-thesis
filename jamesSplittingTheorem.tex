...
\begin{definition}\label{def:suspWedgeSmash}
    ...
    \begin{itemize}
        ...
        Note that if $X$ is pointed, then $\Sigma X$ is canonically pointed as the maps $i_0,i_1\colon*\to\Sigma X$ are equivalent.
        This due to the fact that a map $*\to X$ is a section of the map $X\to *$, and the fact that the canonical maps $X\to *\xrightarrow{i_0}\Sigma X$ and $X\to *\xrightarrow{i_1}\Sigma X$ are equivalent.
...
\begin{lemma}\label{lem:assocWedge}
    Let $D$ be an \inftycat/.
    Then the wedge sum is associative in the sense that for pointed objects $A,B,C$ of $D$ there is a canonical equivalence $(A\vee B)\vee C\simeq A\vee (B\vee C)$ of pointed objects.
...
\begin{lemma}\label{lem:productPreservesPushout}
    Let $T$ be an \inftytop/, $X$ an object in $T$ and
    \begin{center}
        \begin{tikzcd} [sep = 4em]
...
\begin{lemma}\label{lem:smashDist}
    Let $T$ be an \inftytop/.
    Then the smash product distributes over the wedge sum in the sense that for pointed objects $X,A,B$ of $T$ there is a canonical equivalence $(A\vee B)\wedge X\simeq (A\wedge X)\vee (B\wedge X)$ of pointed objects.
    \begin{proof}
...
\begin{lemma}\label{lem:pushoutProdSquareIsPushout}
    Let $T$ be an \inftytop/, $f\colon X\to Y\in T_1$ a morphism and 
    \begin{center}
        \begin{tikzcd} [sep = 4em]
            A \arrow[r] \arrow[d] & B \arrow[d] \\
            C \arrow[r] & D \\
    ...
    \begin{proof}
        ...
        Hence by universality the middle and top square of the stacked cube are pushouts.
        ...
        \begin{center}
            $\begin{tikzcd} [sep = 4em]
                ...
                C\times Y \arrow[r] & C\times Y\cup_{A\times X}B\times X
            \end{tikzcd}
            \hspace{-1.6em}\text{and}\hspace{1em}
            \begin{tikzcd} [sep = 4em]
...
\begin{lemma}\label{lem:assocSmash}
    Let $T$ be an \inftytop/.
    Then the smash product is associative in the sense that for pointed objects $A,B,C$ of $T$ there is a canonical equivalence $(A\wedge B)\wedge C\simeq A\wedge (B\wedge C)$ of pointed objects.
    \begin{proof}
        ...
        The analogous construction when starting with the pushout square
        \begin{center}
            \begin{tikzcd} [sep = 4em]
        ...
        \end{center}
        commutes.
        Thus the indicated universal map $(A\wedge B)\wedge C\to A\wedge(B\wedge C)$ is an equivalence of pointed objects, proving the lemma.
    \end{proof}
\end{lemma}
...
\begin{lemma}\label{lem:existenceOfPoSq}
    ...
    \begin{proof}
        ...
        Since $i_0$ and $i_1$ are equivalent maps in $C$ (see \cref{def:suspWedgeSmash}), we get that
        \begin{center}
            \begin{tikzcd} [sep = 4em]
                \Omega\Sigma X \arrow[r] \arrow[d] & * \arrow[d, "i_0"] \\
                * \arrow[r, "i_1"] & \Sigma X \\
        ...
        Then the functor $\lim\colon\Fun(\Lambda_2^2,C)\to\Fun(\Delta^1\times\Delta^1,C)_{pb}$ is an equivalence of \inftycats/ since it is essentially surjective by definition and fully faithful since it is a right Kan-extension functor along the inclusion $\Lambda_2^2\subset\Delta^0\star\Lambda_2^2\cong\Delta^1\times\Delta^1$.
...
\begin{remark}\label{rmk:mapNotProj}
    ...
    If we assume $\alpha=\pi_2$, by universality we have that 
    \begin{center}
        \begin{tikzcd} [sep = 4em]
...
\begin{corollary}\label{lem:poOfProductIsSuspension}
    ...
    that is a pushout.
    \begin{proof}
        This follows from \cref{lem:existenceOfPoSq} and the pasting law applied to 
        \begin{center}
            \begin{tikzcd} [sep = 4em]
                X\times\Omega\Sigma X \arrow[r, "\pi_2"] \arrow[d, "\alpha"'] & \Omega\Sigma X \arrow[d] \\
...
\begin{lemma}\label{lem:poSqWithFactoringTerminal}
    ...
    \begin{proof}
        ...
        \begin{center}
            $\begin{tikzcd}[sep = 3em, column sep = 1.5em]
...
\begin{lemma}\label{lem:allSqArePo}
    ...
    \begin{center}
        \begin{tikzcd} [sep = 4em]
            X\times Y \arrow[r, "\pi_2"] \arrow[d, "\pi_1"'] \ar[dr,phantom,"\Circled{1}"] & Y \arrow[r] \arrow[d] \ar[dr,phantom,"\Circled{2}", pos = .56] & * \arrow[d] \\
...
\begin{lemma}\label{lem:suspensionCommutesWithSmash}
    ...
    \begin{proof}
        Let $i_0,i_1\colon *\to\Sigma Y$ be the canonical maps from the definition of $\Sigma Y$ as a pushout.
...
\begin{corollary}[Fundamental James Splitting]\label{cor:fundamentalJamesSplitting}
    ...
    \begin{proof}
        Applying \cref{lem:allSqArePo} to $X$ and $Y=\Omega\Sigma X$, the big left square $\Circled{1}+\Circled{3}$ of \cref{lem:allSqArePo} is equivalent to the square 
        \begin{center}
            \begin{tikzcd} [sep = 4em]
                X\times\Omega\Sigma X \arrow[r, "\pi_2"] \arrow[d] & \Omega\Sigma X \arrow[d] \\
                * \arrow[r] & \Sigma\Omega\Sigma X \\
...
We will give a recursive definition of connectedness of maps similar to the definition of truncatedness (\cref{def:truncatedMap}).
Also note that in view of \cref{prop:conn} \ref{prop:connSection}, the previously introduced notion of a pointed connected object (\cref{def:catOfPtdConnObj}) agrees with the definition of a $0$-connected object that is pointed.
\begin{definition}\label{def:connected}
    ...
    We consider an object $X$ to be \emph{$k$-connected} for $0\leq k\leq\infty$ if the map $X\to *$ is $k$-connected.
\end{definition}
\begin{remark}
    ...
    \begin{itemize}
        ...
        \item \emph{topologically $\infty$-connected} if it is a weak equivalence (hence $n$-connected for all $n\in\Nat\cup\set{-1}$)
    \end{itemize}
    with the convention that all spaces are \emph{topologically $(-2)$-connected}.
    A map $f\colon X\to Y$ in $\Top$ is \emph{topologically $n$-connected} for $-2\leq n\leq\infty$ if all its homotopy fibers are $n$-connected. 
    (Some authors take this to be the definition of a topologically $(n+1)$-connected map.)
    
    This definition agrees with the definition of connectedness in $\spaces$:
    \begin{itemize}
        \item For $n=-2$ there is nothing to prove.
        \item For $n\geq -1$ all homotopy fibers of $f$ are nonempty, hence $f$ is an effective epimorphism (see \cref{rmk:emptyFiber} and \cref{prop:effInSpaceIffSurj}).
...
However this case is sufficient to derive the James Splitting Theorem for general \inftytops/ (see \cref{lem:genTopoiComparisonMapEq}).
...
\begin{prop}\label{prop:conn}
    ...
    \end{enumerate}
    \begin{reference}
        \cite[Corollary 6.5.1.17]{HTT} for \ref{prop:connStableUnderPo}, \cite[Proposition 6.5.1.16]{HTT} for \ref{prop:connStableUnderPb} and \ref{prop:connRightCancel}, \cite[Proposition 6.5.1.20]{HTT} for \ref{prop:connSection} and \cite[Corollary 4.11]{splittings_21} for \ref{prop:connStableFilteredColim}.
    \end{reference}
\end{prop}
\begin{thm}[Blakers-Massey Theorem]\label{prop:blakersMassey}
    Let $C$ be an \inftytop/, $m,n\geq-1$ and let
    \begin{center}
        \begin{tikzcd} [sep = 4em]
    ...
    \begin{reference}
        ...
        A proof this way can be found in \cite[Proposition 8.16]{toposes_and_htpy_toposes}.
    \end{reference}
\end{thm}
With these we can deduce the following statements:
\begin{corollary}\label{cor:connEst} 
    Let $C$ be an \inftytop/, $X,Y$ pointed objects in $C$ and $k,l\geq-1$.
    ...
    \begin{enumerate}[label={(\roman*)}]
        ...
        \item for every $n\geq 1$, the smash product $X^{\wedge n}$ is $(n(k+1)-1)$-connected. \label{cor:connEstIteratedSmash}
    \end{enumerate}
    \begin{proof}
    ...
    \end{proof}
\end{corollary}
\begin{prop}[James Splitting for Presheaf Categories]\label{prop:jamesSplittingPresheaf}
    ...
    \begin{proof}
        The proof follows \cite[Proposition 4.18]{splittings_21}.
        
        Since $\Fun(A,\spaces)$ is hypercomplete it suffices to prove that $c_X$ is $\infty$-connected.

        First, for each $n\in\Nat_{\geq 1}$ we have a $2$-simplex
        \begin{center}
            \begin{tikzcd} [sep = 4em]
            ...
            \end{tikzcd}
        \end{center}
        in $\Fun(A,\spaces)$ induced by the universal property of $\bigvee\limits_{i=1}^{\infty}\Sigma X^{\wedge i}$.

        By \cref{cor:connEst} \ref{cor:connEstIteratedSmash} $X^{\wedge n}$ is $(n-1)$-connected.
        By \cref{prop:conn} \ref{prop:connStableUnderPo} and \cref{prop:conn} \ref{prop:connStableUnderPb} we have that $\Omega\Sigma X$ is $0$-connected, so by \cref{cor:connEst} \ref{cor:connEstSuspension} $\Sigma\Omega\Sigma X$ is $1$-connected.
        Combining this via \cref{cor:connEst} \ref{cor:connEstSmash} shows that $X^{\wedge n}\wedge\Sigma\Omega\Sigma X$ is $(n+1)$-connected.
        ...
        As the diagonal as well as the horizontal map of the triangle above are $n$-connected, it follows from \cref{prop:conn} \ref{prop:connRightCancel} that $c_X$ is $n$-connected. 
        As $n$ was arbitrary, $c_X$ is $\infty$-connected and therefore an equivalence by hypercompleteness of $\Fun(A,\spaces)$.
    \end{proof}
\end{prop}
...
\begin{thm}\label{thm:charTopoiByAdj}
    ...
    \end{reference}
\end{thm}
Furthermore we need to be able to transfer statements about pointed connected objects between the \inftytops/ $\Fun(A,\spaces)$ and $C$ of the previous theorem.
We will do this using the next proposition.
\begin{prop}[Epi/Mono Factorization]\label{prop:factorConnTrunc}
    Let $C$ be an \inftytop/.
    Then we can factor every map $f\colon X\to Y$ in $C$ into a $(-1)$-connected map followed by a $(-1)$-truncated map.

    Furthermore, a map that is both $(-1)$-connected and $(-1)$-truncated is an equivalence.
    \begin{proof} 
        The first part of the proof follows \cite[21]{leeds_lectures}.

        Let $U=\CN(f)|_{\N(\Delta)^{\op}}$ be the restriction of the \Cech/ nerve of $f$, let $E=\colim\limits_{\N(\Delta)^{\op}}U$ and let $U^+\colon\N(\Delta_+)^{\op}\to C$ denote the corresponding colimit diagram.
        Then we obtain a canonical factorization $X\to E\to Y$ of $f$ where $X\to E$ is an effective epimorphism by \cref{prop:grpdEffectiveIfPullback} and \cref{prop:mayRecognitionTheoremGroupoid}.
        We claim that $E\to Y$ is $(-1)$-truncated:

        We need to show that the diagonal map $E\to E\times_YE$ is an equivalence, but for this it suffices to show that one of the projections $E\times_YE\to E$ is an equivalence.
        By universality we only need to show that the projection $E\times_YU_n\to U_n$ is an equivalence for all $n\geq 0$.
        By induction and using \cref{rmk:cechNerveExplicit} we see that we only need to prove the case $n=0$, hence that the projection $E\times_Y A\to A$ is an equivalence.

        Setting $V=U^+\times_Y A\colon\N(\Delta_+)^{\op}\to C$ (which is again a colimit diagram by universality), we note that $V|_{\N(\Delta)^{\op}}\simeq (U\circ\N(F)^{\op})|_{\N(\Delta)^{\op}}$ using the map $F\colon\Delta_+\to\Delta$ from \cref{prop:shiftIsColimDiam}.
        \Cref{prop:shiftIsColimDiam} implies that $E\times_Y A\simeq V_{-1}\simeq\colim\limits_{\N(\Delta)^{\op}}V|_{\N(\Delta)^{\op}}\simeq(U\circ\N(F)^{\op})_{-1}= U_0\simeq A$.
        Since this map $E\times_YA\to A$ is the canonical projection the claim follows.
        
        Lastly we prove that a map $f\colon X\to Y$ which is both $(-1)$-connected and $(-1)$-truncated is an equivalence. 
        The proof follows \cite[Lemma 6.2.3.16]{HTT}.

        Since $f$ is $(-1)$-truncated, the projections in the pullback diagram
        \begin{center}
            \begin{tikzcd}[sep = 4em]
                X\times_YX \ar[r, "\pi_1", "\simeq"'] \ar[d, "\pi_2"', "\simeq"] & X \ar[d, "f"] \\
                X \ar[r, "f"] & Y
            \end{tikzcd}
        \end{center}
        are equivalences. 
        This pullback diagram induces a natural transformation $\alpha\colon\CN(\pi_1)\to\CN(f)$.
        Using \cref{rmk:cechNerveExplicit} and the pasting law, we obtain that for all $n\geq 1$ the left square of the diagram
        \begin{center}
            \begin{tikzcd}[sep = 4em]
                X\times_Y(X\times_Y\ldots\times_YX)\simeq\CN(\pi_1)_n \ar[r, "{(\pi_1,\pi_2)}"] \ar[d, "\alpha_n\simeq{(\pi_2,\ldots,\pi_{n+2})}"'] & X\times_YX\simeq\CN(f)_0 \ar[d, "\alpha_0\simeq\pi_2",  "\simeq"'] \ar[r, "\pi_1"] & X \ar[d, "f"]\\
                X\times_Y\ldots\times_YX\simeq \CN(f)_n \ar[r, "\pi_1"] & X\simeq\CN(\pi_1)_0 \ar[r, "f"] & Y
            \end{tikzcd}
        \end{center}
        is a pullback square, which shows that for $n\geq0$ the maps $\alpha_n$ are equivalences.
        Thus $\alpha|_{\N(\Delta)^{\op}}$ is an invertible natural transformation.
        As $f$ is an effective epimorphism, $\pi_1$ is as well by \cref{prop:conn} \ref{prop:connStableUnderPb}.
        Hence both $\CN(\pi_1)$ and $\CN(\pi_1)$ are colimit diagrams and thus $\alpha_{-1}$ is an equivalence.
        Since $\alpha_{-1}\simeq f$ the claim follows.
    \end{proof}
\end{prop}
\begin{remark}
    The statement of \cref{prop:factorConnTrunc} generalizes to all $-2\leq m<\infty$:
    ...
    The name Epi/Mono factorization derives from the fact that $(-1)$-truncated morphisms are also referred to as \emph{monomorphisms} and $(-1)$-connected maps are precisely the effective epimorphisms.
    It is analogous to the factorization of a map in $\Set$ into an epimorphism followed by a monomorphism over the image.
\end{remark}
\begin{corollary}[James Splitting for \inftyTops/]\label{lem:genTopoiComparisonMapEq}
    ...
    \begin{proof}
        ...
        Then by \cref{prop:conn} \ref{prop:connSection} the map $x\colon*\to X$ is $(-1)$-connected.
        ...
        Thus the map $L(y)$ is $(-1)$-connected (in particular, $L(Y)$ is a $0$-connected object), and by \cref{prop:conn} \ref{prop:connRightCancel} the map $L(g)$ is $(-1)$-connected as well as $(-1)$-truncated, hence an equivalence by \cref{prop:factorConnTrunc}.
        This shows that $X$ and $L(Y)$ are equivalent as pointed objects.

        Since $L$ preserves colimits and finite limits, it preserves all colimits and limits appearing in $c_Y$, thus we have $L(c_Y)\simeq c_{L(Y)}$ in $C_*$.
        But since $L(Y)\simeq X$ in $C_*$, this means that $c_{L(Y)}\simeq c_X$ in $C_*$ which proves that $c_X$ is an equivalence in $C_*$.
    \end{proof}
\end{corollary}
...
\begin{lemma}\label{lem:reducedHomLoopOfSpheres}
    ...
    \begin{proof}
        ...
        By \cref{cor:classicalJamesSplitting} we therefore have an isomorphism 
        \begin{equation*}
...
\end{lemma}
\begin{corollary}
    Let $H_*$ be reduced ordinary homology and let $S^n$ with $n\geq1$ be a model of the pointed $n$-sphere.
    Then we have
    \begin{equation*}
