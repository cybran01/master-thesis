...
\end{definition}
% \begin{remark}
%     In $\spaces$ this definition agrees (up to an index shift depending on convention) with the usual notion of connectedness. 
%     %TODO elaborate %TODO prove this agrees with standard def (after shift of index at least), HTT Proposition 6.5.1.16 (6) Proposition 6.5.1.20. for section
% \end{remark}
\begin{remark}
    ...
    \begin{itemize}
        ...
        \item for $k\geq -1$ the fiber is inhabited for all chosen basepoints and thus $f$ is an effective epimorphism (see \cref{prop:effInSpaceIffSurj}).
            Since homotopy fibers are essentially constant on connected components, this allows us to prove that $f$ is $k$-connected if $g$ is $k$-connected (the other implication does not need $f$ to be an effective epimorphism and follows directly from the fiberwise characterization of homotopy pullbacks). %TODO maybe say how to do this
    \end{itemize}
...
\begin{remark}
    ...
    The \inftytop/ $\spaces$ is hypercomplete. %TODO reference HTT 
\end{remark}
Our goal now is to prove that the comparison map is an equivalence whenever $X$ is a $0$-connected pointed object. %TODO define k-connected objects, perhaps at definition of truncation
...
\begin{prop}\label{prop:conn}
    ...
    \end{enumerate}
    \begin{proof}
        %TODO Proposition 4.10. in paper
    \end{proof}
\end{prop}
\begin{prop}[Blakers-Massay Theorem]\label{prop:blakersMassay}
    ...
    \begin{proof}
        %TODO gen_blakers_massay Corollary 4.3.1
    \end{proof}
\end{prop}
...
\begin{corollary}\label{cor:connEst}
    ...
    \end{proof}
\end{corollary}
% \begin{corollary}[Freudenthal Suspension Theorem]\label{cor:freudenthalSuspension}
%     Let $C$ be an \inftytop/ and let $X$ be a pointed object that is $n$-connected.
%     Then the universal morphism $X\to\Omega\Sigma X$ is $2n$-connected.
%     \begin{proof}
%         This is \cref{prop:blakersMassay} applied to the square 
%         \begin{center}
%             \begin{tikzcd} [sep = 4em]
...
\begin{prop}[James Splitting for Presheaf Categories]\label{prop:jamesSplittingPresheaf} %ref Proposition 4.18 
    ...
    \begin{proof}
        ...
        By \cref{prop:conn} \ref{prop:connStableUnderPo} and \cref{prop:conn} \ref{prop:connStableUnderPo} we have that $\Omega\Sigma X$ is $0$-connected, so by \cref{cor:connEst} \ref{cor:connEstSmash} $\Sigma\Omega\Sigma X$ is $1$-connected and combining this via \cref{cor:connEst} \ref{cor:connEstSmash} shows that $X^{\wedge n}\wedge\Sigma\Omega\Sigma X$ is $(n+1)$-connected.
        ...
        Since by \cref{cor:connEst} \ref{cor:connEstIteratedSmash} and \cref{cor:connEst} \ref{cor:connEstSmash} every object of the filtered colimit $\bigvee\limits_{i=n+1}^{\infty}\Sigma X^{\wedge i}$ is at least $(n+1)$-connected, by \cref{cor:connEst} \ref{prop:connStableFilteredColim} it is iself $(n+1)$-connected.
...
\begin{remark}\label{rem:genTopoiComparisonMapEq}
    Due to the equivalent characterizations of \inftytops/ as in %TODO HTT
    ...
    Then if for a pointed object $X\in\left(C_*\right)_0$ the comparison morphism $c_{i(X)}$ is an equivalence in $Fun(A,\spaces)$, so is $c_X$ in $C$ (see %TODO paper, Lemma 4.17.
    ).
    If $X$ is $0$-connected then so is $i(X)$, so it suffices to prove that it $c_{i(X)}$ is $\infty$-connected since $\spaces$ is hypercomplete and $Fun(A,\spaces)$ inherits this property. %TODO define hypercomplete %TODO prove that X 0-conn => i(X) 0-conn
