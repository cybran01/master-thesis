...
\begin{definition}
    ...
    \begin{itemize}
        ...
        Note that since both canonical maps $X\to *\to \Sigma X$ are equivalent, when $X$ is pointed then so is $\Sigma X$ via the map $*\to X\to\Sigma X$. %TODO do i need this?
...
\begin{lemma}\label{lem:assocWedge}
    ...
    In fact, $(A\vee B)\vee C\simeq(A\vee B)\cup_{B}A\vee C\simeq A\vee (B\vee C)$ as pointed objects.
    \begin{proof}
...
As a first step, we will show that for pointed objects $X$ there is a canonical equivalence $\Sigma\Omega\Sigma X\cong\Sigma X\vee\left(X\wedge\Sigma\Omega\Sigma X\right)$ of pointed objects.
...
\begin{lemma}\label{lem:existenceOfPoSq}
    ...
    \begin{proof}
        ...
        The previous diagram corresponds to a natural transformation $\alpha\in\Fun(\Lambda_2^2,C)_1$ where $\alpha_0$ and $\alpha_1 $ are the restrictions of the back square and the front square to $\Lambda_2^2$ respectively.
...
\begin{remark}
    ...
    Taking $X=S^0$ in $\spaces$ as an example, we have that $c_{S^0}$ simplifies to the map
    \begin{equation*}
        \bigvee\limits_{i=1}^n S^1\to\bigvee\limits_{\Z}\Sigma\Omega S^1
    \end{equation*}
