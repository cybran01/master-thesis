...
\begin{thm}[Blakers-Massey Theorem]\label{prop:blakersMassey}
    Let $C$ be an \inftytop/, $m,n\geq-1$ and
    \begin{center}
        \begin{tikzcd} [sep = 4em]
...
\begin{prop}[Epi/Mono Factorization]\label{prop:factorConnTrunc}
    ...
    \begin{proof} 
        ...
        Then \cref{prop:shiftIsColimDiam} implies that $E\times_Y A\simeq V_{-1}\simeq\colim\limits_{\N(\Delta)^{\op}}V|_{\N(\Delta)^{\op}}\simeq(U\circ\N(F)^{\op})_{-1}= U_0\simeq A$.
        Since this map $E\times_YA\to A$ is the canonical projection the claim follows.
        
        Lastly we prove that a map $f\colon X\to Y$ that is both $(-1)$-connected and $(-1)$-truncated is an equivalence. 
...
\begin{remark}
    ...
    It is analogous to the factorization of a map into epimorphism follows by a monomorphism in $\Set$ over the image.
\end{remark}
\begin{corollary}[James Splitting for \inftyTops/]\label{lem:genTopoiComparisonMapEq}
    ...
    \begin{proof}
        ...
        Since $L$ preserves colimits and finite limits, it preserves all colimits and limits appearing in $c_Y$, hence $L(c_Y)\simeq c_{L(Y)}$ in $C_*$.
