...
\begin{definition}
    ...
    \begin{itemize}
        ...
        Note that if $X$ is pointed, then $\Sigma X$ is canonically pointed by the essentially unique map $*\to X\to\Sigma X$ since the canonical maps $X\to *\xrightrightarrows[i_1]{i_0}\Sigma X$ are equivalent.
...
\begin{lemma}\label{lem:existenceOfPoSq}
    ...
    \begin{proof}
        ...
        Then the functor $\lim\colon\Fun(\Lambda_2^2,C)\to\Fun(\Delta^1\times\Delta^1,C)_{pb}$ is an equivalence of \inftycats/ since it is essentially surjective by definition and fully faithful since it is a right Kan-extension along the inclusion $\Lambda_2^2\subset\Delta^1\times\Delta^1$.
...
\begin{prop}[Epi/Mono Factorization]\label{prop:factorConnTrunc}
    ...
    \begin{proof} 
        ...
        For all $n\geq 1$ using \cref{rmk:cechNerveExplicit} and the pasting law, we obtain the left square of the diagram
        \begin{center}
            \begin{tikzcd}[sep = 4em]
