...
\begin{prop}[Epi/Mono Factorization]\label{prop:factorConnTrunc}
    ...
    \begin{proof} 
        ...
        \end{center}
        are equivalences. 
        This pullback diagram induces a natural transformation $\alpha\colon\CN(\pi_1)\to\CN(f)$ which can be shown to be cartesian using \cref{rmk:cechNerveExplicit} and the pasting law.
        For all $n\geq 1$ the map $\alpha_n$ is a pullback of $\pi_2\simeq\alpha_0$, thus $\alpha|_{\N(\Delta)^{\op}}$ is an equivalence.
        As $f$ is an effective epimorphism and $\alpha$ is cartesian, $\CN(\pi_1)$ is also a colimit diagram by universality and thus $\alpha_{-1}$ is an equivalence.
...
\begin{remark}
    ...
    It is analogous to the factorization of maps into epimorphisms and monomorphisms in $\Set$ over the image.
\end{remark}
