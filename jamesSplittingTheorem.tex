...
\begin{remark}\label{rem:genTopoiComparisonMapEq}
    ...
    If $X$ is $0$-connected then so is $i(X)$, so it suffices to prove that it $c_{i(X)}$ is $\infty$-connected since $\spaces$ is hypercomplete and $Fun(A,\spaces)$ inherits this property. %TODO prove that X 0-conn => i(X) 0-conn
