...
\begin{lemma}\label{lem:existenceOfPoSq} %TODO ON THE JAMES AND HILTON–MILNOR SPLITTINGS, & THE METASTABLE EHP SEQUENCE lemma 2.17
    ...
    \begin{proof}
        ...
        \begin{center}
            ...
            \end{tikzcd}
        \end{center}
        shows that $i_1$ and $i_2$ are equivalent maps in $C$, since $X\to *\xrightarrow{i_1}\Sigma X$ and $X\to *\xrightarrow{i_2}\Sigma X$ are identical in $\ho(C)$ by construction and $X\to *$ is an epimorphism in $\ho(C)$ since $*$ is terminal. %TODO this proof is incorrect (it is not necessarily an epi); fix!
        Thus we get that 
        \begin{center}
            \begin{tikzcd} [sep = 4em]
