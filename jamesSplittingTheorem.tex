    ...
    \begin{proof}
        ...
        \begin{center}
            \begin{tikzcd} [sep = .5 cm]
                X\times\Omega\Sigma X \arrow [rr, "\pi_2"] \arrow [dd] & & \Omega\Sigma X \arrow [dr] \arrow[dd] \\
            ...
            \end{tikzcd}
        \end{center}
        where tree vertical squares are pullback and the bottom square is a pushout.
        We now want to construct a map $\alpha\colon X\times\Omega\Sigma X\to\Omega\Sigma X$ such that we have a commutative cube vertical sides pullback.
...
\begin{remark}
    ...
    If we assume $\alpha=\pi_2$ then by universality have that 
    \begin{center}
        \begin{tikzcd} [sep = 4em]
