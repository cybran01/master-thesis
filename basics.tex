...
We also assume familiarity with (proper) model categories and the computation of homotopy limits/colimits (in particular homotopy pullbacks/pushouts and homotopy products/coproducts). %TODO say that we take these to be defined via derived functors
...
\begin{remark}[The Boardmann-Vogt Construction]\label{rmk:boardmanVogt}
    For an \inftycat/ $C$ there is a explicit construction of $\ho(C)$ due to Boardman and Vogt (see e.g. \cite[Theorem 1.6.6]{cisinski_2019} for details).
    We will later introduce a simplicial set $\Map_C(x,y)$ representing maps between objects $x,y\in C_0$ (see \cref{def:mappingSpace}).
    The sets of maps $\hom_{\ho(C)}(x,y)$ is then a quotient of $\Map_C(x,y)_0$, hence we can send a map $f\colon\Delta^1\to C\in\Map_C(x,y)_0$ to its class $[f]\in\hom_{\ho(C)}(x,y)$.
\end{remark}
...
\begin{definition}[Saturated Class]
    ...
    Then we say that $I$ is \emph{saturated} if it is closed under retracts, pushouts and transfinite composition. %TODO perhaps find way to avoid transfinite composition
\end{definition}
...
\begin{definition}[Properness] %TODO maybe diagrams
    ...
    If the pushout of weak equivalences along cofibrations are again weak equivalences, we say that $M$ is \emph{left proper}.
    If the pullback of weak equivalences along fibrations are again weak equivalences, we say that $M$ is \emph{right proper}.
...
\begin{prop}
    ...
    \end{reference}
\end{prop}
%TODO stress importance & mention 1-categorical case
\begin{prop}[Pasting Law]
    ...
    \end{reference}
\end{prop}
%TODO maybe equivalences of squares here (reference?) (is this really relevant, since it is closely related to how htpy pb/po are defined)
