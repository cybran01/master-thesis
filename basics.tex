...
\begin{definition}[(Augmented) Simplex Category]
    ...
    Furthermore, for $0\leq i\leq n$ we call the unique nondecreasing injective map $d_i^n\colon[n-1]\to[n]$ such that $i\notin\im d_i^n$ the \emph{$i$-th face map in dimension $n$}, and for $-1\leq i\leq n$ we call the unique nondecreasing surjective map $s_i^n\colon[n+1]\to[n]$ such that $s_i^n(i)=s_i^n(i+1)$ the \emph{$i$-th degeneracy map in dimension $n$}.
\end{definition}
...
\end{remark}
\begin{definition}[Nerve of an Ordinary Category] %smallness problematic when applied to sSet and Top? (did i solve it already?)
...
\begin{prop}[Existence of Pointwise Kan-Extensions]\label{prop:exKanExt}
    ...
    If $D$ admits $\left(C_{c/}\times_C C_0\right)$-shaped limits, then there exists a right Kan-extension $i_*\colon\Fun(C_0,D)\to\Fun(C,D)$ of $i$ such that for every map $f\colon C_0\to D$ the induced map $i_*(f)$ is equivalent to a pointwise right Kan-extension along $i$.
    Furthermore, the counit in the adjunction is an equivalence hence $i_*$ is fully faithful.

    Dually if $D$  $\left(C_{c/}\times_C C_0\right)$-shaped colimits, then there exists a left Kan-extension $i_!\colon\Fun(C_0,D)\to\Fun(C,D)$ of $i$ such that for every map $f\colon C_0\to D$ the induced map $i_!(f)$ is equivalent to a pointwise left Kan-extension along $i$.
    Furthermore, the unit in the adjunction is an equivalence hence $i_!$ is fully faithful..
    \begin{reference}
        ...
        One can then use \cref{rmk:homAdjunction} to prove that this is equivalent to full faithfulness by essentially the same argument as in ordinary category theory.
    \end{reference}
\end{prop}
