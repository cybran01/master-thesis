...
\begin{definition}[$\kappa$-Filtered Category]
    ...
    If the cardinal $\kappa$ is omitted, we let $\kappa=\aleph_0=|\Nat|$.
\end{definition}
...
\begin{definition}[Kan-Extension]
    ...
    If the functor $u^*\colon\Fun(B,C)\to\Fun(A,C)$ has a right adjoint $u_*\colon\Fun(A,C)\to\Fun(B,C)$, we say that $u_*$ is a \emph{right Kan-extension functor along $u$ for $C$}.
...
\begin{definition}[Pointwise Kan-Extension]\label{def:ptwiseKanExt} %kerodon Definition 7.3.2.1
    ...
    Then $F$ is a \emph{pointwise right Kan-extension along $i$} if for all objects $c\in C_0$ the diagram
    \begin{equation*}
    ...
    Dually, $F$ is a \emph{pointwise left Kan-extension along $i$} if for all objects $c\in C_0$ the diagram
    \begin{equation*}
...
\begin{prop}[Existence of Pointwise Kan-Extensions]\label{prop:exKanExt}
    ...
    If $D$ admits $\left(C_{c/}\times_C B\right)$-shaped limits for all $c\in C_0$, then there exists a fully faithful right Kan-extension functor $i_*\colon\Fun(B,D)\to\Fun(C,D)$ along $i$ such that for every map $f\colon B\to D$ the induced map $i_*(f)$ is equivalent to a pointwise right Kan-extension along $i$.

    Dually if $D$ admits $\left(C_{c/}\times_C B\right)$-shaped colimits for all $c\in C_0$, then there exists a fully faithful left Kan-extension functor $i_!\colon\Fun(B,D)\to\Fun(C,D)$ along $i$ such that for every map $f\colon B\to D$ the induced map $i_!(f)$ is equivalent to a pointwise left Kan-extension along $i$.
    \begin{reference}
