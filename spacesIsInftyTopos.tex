...
\begin{prop}%TODO cite quasifibs Thm 1.2
    ...
    \begin{proof}
        ...
        We will use a nested induction argument, where the outer induction runs through the intervalls $I_r$ with $r\in\set{0,\ldots,s-1}$ and for fixed $r$ the inner induction runs through $m\in\set{0,\ldots,n}$ over the collections $\Gamma^m$.

        So for the induction step with given $r$ and $m$, set $L_r^m=\left([0,v_r]\times I^n\right)\cup J^n\cup \bigcup\limits_{K\in\Gamma^m}\left(I_r\times K\right)\subset I^{n+1}$.
        Then by the induction hypothesis we have maps $G\colon L_r^m\to X$ and $H\colon L_r^m\times I\to Y$ extending $g$ and the constant homotopy at $fg=e|_{J^n}$ respectively such that $H$ is a homotopy from $e|_{L_r^m}$ to $fG$.
        Furthermore these maps fulfill the two conditions above.
        ...
        Together with $e'=e|_{I_r\times K}\colon(I^{m+1},\set{1}\times I^m)\to(Y_j,Y_1\cap Y_2)$ and the equivalence of pairs $f\colon(X_j,X_1\cap X_2)\to(Y_j,Y_1\cap Y_2)$ these fulfill the conditions of \cref{prop:eqCharInjSurj} \ref{itm:liftingMapsAndHtpy}. 
        ...
        In the remaining cases either $e(\set{v_{r+1}}\times K)\not\subset Y_1\cap Y_2$ or $e(I_r\times K)\subset Y_1^°\cap Y_2^°$ and we can pick a deformation retract $F\colon I_r\times K\times I\to I_r\times K$ of $I_r\times\partial K\cup\set{v_r}\times K$ by TODO %TODO
        and set $G'=GF_1$ and $H'=eF*HF_1$.
    ...
    \end{proof}
\end{prop}
%TODO remark that this is essentially already descent for po
\begin{corollary}[Locality of Quasifibrations]\label{cor:locOfQuasifib}
    %TODO
\end{corollary}
...
\begin{prop}\label{lem:topDescentPo}
    ...
    \begin{proof}
        ...
        \begin{center}
            ...
            \end{tikzcd}
        \end{center}
        where the indicated maps are h-cofibrations as they are either inclusions into the mapping cylinder (see e.g. \cite[Theorem 2]{note_on_cofibs_1}) or pushouts of such.
