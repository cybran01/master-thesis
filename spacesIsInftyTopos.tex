...
The proof of descent will be more involved then the proof of universality.
The main theorem will be \cref{prop:descentForPairs}. 
There are two routes that lead from this theroem to descent for pushouts:
...
We will showcase both; 
the first since it is very direct and the second since descent is closely related to locality as \cref{rmk:locOfQuasifibDescent} demonstrates.
...
\end{definition}
As expected we get a canonical group structure on these sets for $n\geq 2$ (that is even abelian for $n\geq 3$), but we will not need this.
...
\begin{remark}\label{rmk:locOfQuasifibDescent}
    Once we have established descent in $\Top$ in full generality (\cref{cor:spacesIsInftyTop}) we obtain a stronger version of \cref{cor:locOfQuasifib} which is often called ``Locality of Quasifibrations'':
