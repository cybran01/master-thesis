...
\begin{lemma}[Relative HEP]\label{lem:rHEP}
    ...
    \end{proof}
\end{lemma}
%TODO add all used triads (I^{n+1};J^n,\set{1}\times I^n) as examples HERE, and maybe also deformation retract $J^n\subset I1{n+1}$
\begin{definition}
...
\begin{prop}\label{prop:descentForPairs} %TODO cite quasifibs Thm 1.2
    ...
    \begin{proof}
        ...
        We will construct the desired maps $G$ and $H$ on the spaces $I_r\times K$ where $K$ is a cell of the CW-structure induced by $\mathcal{D}$ on $I^n$.
        These maps will be constructed such that for all cells $K$ and $r\in\set{0,\ldots,s-1}$
        \begin{itemize}
        ...
        Let $\Gamma^m$ denote the collection of cells of the CW-structure on $I^n$ whose dimension is $m$ for $m\in\set{0,\ldots,n}$ and whose intersection with $(I^n)^°$ is nonempty.
        We will use a nested induction argument, where the outer induction runs through the intervalls $I_r$ with $r\in\set{0,\ldots,s-1}$ and for fixed $r$ the inner induction runs through $m\in\set{0,\ldots,n-1}$ over the collections $\Gamma^m$ to extend cells to $\Gamma^{m+1}$.

        So for the induction step with given $r\in\set{0,\ldots,s-1}$ and $m\in\set{0,\ldots,n-1}$, set $L_r^m=\left([0,v_r]\times I^n\right)\cup J^n\cup \bigcup\limits_{K\in\Gamma^m}\left(I_r\times K\right)\subset I^{n+1}$ for $m\geq 1$ and $L_r^0=\left([0,v_r]\times I^n\right)\cup J^n\subset I^{n+1}$ else.
        ...
        We now pick a $K\in\Gamma^{m+1}$ and proceed via a case distinction:
        ...
        So by \cref{prop:eqCharInjSurj} there exist extensions $G'$ and $H'$ which we can then identify with maps $(I_r\times K,\set{v_{r+1}}\times K)\to(X_j,X_1\cap X_2)$ and $(I_r\times K,\set{v_{r+1}}\times K)\times I\to(Y_j,Y_1\cap Y_2)$ fulfilling the desired properties.

        In the remaining cases either $e(\set{v_{r+1}}\times K)\not\subset Y_1\cap Y_2$ or $e(I_r\times K)\subset Y_1^°\cap Y_2^°$.
        We can pick a deformation retract $F\colon I_r\times K\times I\to I_r\times K$ of $I_r\times\partial K\cup\set{v_r}\times K$ by TODO %TODO
        and set $G'=GF_1$ and $H'=eF*HF_1$ which considered as maps $(I_r\times K,\set{v_{r+1}}\times K)\to(X_j,X_1\cap X_2)$ and $(I_r\times K,\set{v_{r+1}}\times K)\times I\to(Y_j,Y_1\cap Y_2)$ respectively fullfill the desired properties.

        Now note that since we work ourselves upwards from the dimensions and keep the maps fixed on $J^n$, the constructed maps $G'$ and $H'$ are always compatible to the maps from the previous step.
        Hence by doing the induction step for all $K\in\Gamma^{m+1}$ we get extensions of $G\colon L_r^m\to X$ and $H\colon L_r^m\times I\to Y$ to $L_r^{m+1}$ and $L_r^{m+1}\times I$ respectively.
        We also note that $L_r^n=\left([0,v_{r+1}]\times I^n\right)\cup J^n$, thus $L_{s-1}^n=I^{n+1}$.
        ...
        Let $K\in L_{s-1}^n$ and note that $\bigcup\limits_{K\in L_{s-1}^n}\set{1}\times K=\set{1}\times I^n$.
        Since $e(\set{1}\times I^n)\subset Y_i$, we always have $e(I_{s-1}\times K)\subset Y_i^°$ or $e(\set{1}\times K)\subset Y_1\cap Y_2$ and both cases imply $G(\set{1}\times K)\subset X_i$ and $H(\set{1}\times K\times I)\subset X_i$.
