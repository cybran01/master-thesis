...
\begin{lemma}\label{lem:replaceWithStrictDiagram} %TODO is the statement even true in this generality?
    ...
    \begin{equation*}
        \N(\Fun(J,M))\cong\Fun(\N(J),\N(M))\to\Fun(\N(J),\N(M)[W^{-1}]) %TODO is this correct?
    \end{equation*} 
...
\begin{corollary}\label{cor:sufficientToProveInModCat}
    ...
    \end{proof}
\end{corollary}
We will use the interaction of the \Strom/ model structure and the Quillen model structure to prove universality and descent $\Top$.
