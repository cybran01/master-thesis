...
\begin{prop}\label{prop:poAlongHCofibIsHtpyPo}
    ...
    \end{proof}
\end{prop}
%TODO insert  section proof of universality here
...
%TODO insert section about descent of po here
...
\begin{remark}\label{rmk:cwInclIsHCofib}
    ...
    This is because all the required maps can be identified with maps $S^{n-1}\hookrightarrow D^n$ (where $S^{-1}=\emptyset$) which are h-cofibrations.
\end{remark}
\begin{definition}
...
\begin{prop}\label{prop:descentForPairs} %TODO cite quasifibs Thm 1.2
    ...
    \begin{proof}
        ...
        We pick a cubical subdivision $\mathcal{C}$ of $I^{n+1}$ such that the diameter of every cube $c\in\mathcal{C}$ is smaller than $\delta$.
        This ensures that for every cube $c\in\mathcal{C}$ there is an $i\in\set{1,2}$ such that $e(c)\subset Y_i^°$.
...
\begin{prop}\label{prop:topDescentPo}
    ...
    \end{proof}
\end{prop}
%TODO subsection about quasifibs & descent
...
\end{definition}
It turns out that a weak version of the locality of quasifibrations (\cref{cor:locOfQuasifib}) is sufficient to prove descent for pushouts.
This relationship also goes the other way as the next remark shows.
\begin{remark}
    ...
    Let $f\colon X\to Y$ be a map and $(U_i)_{i\in I}$ an open cover such that for every pair of indices $i,j\in I$ and point $b\in U_i\cap U_j$, there is a $k\in I$ such that $b\in U_k\subset U_i\cap U_j$.
...
\end{remark}
The main tool we will use to prove descent of pushouts via quasifibrations is the following.
\begin{corollary}[Weak Locality of Quasifibrations]\label{cor:locOfQuasifib}
    ...
    \end{proof}
\end{corollary}
In order to reduce to a situation where \cref{cor:locOfQuasifib} is applicable we will need to use the next two lemmas.
...
\begin{prop}\label{lem:topDescentPoAlt}
    ...
    \end{proof}
\end{prop}
%TODO section on descent for htpy coprod
