...
\begin{document} %TODO fix quotations ´´''
    ...
    \begin{abstract}\noindent
      ...
      \noindent As an application of descent, we will then prove the May Recognition Theorem in general \inftytops/, which allows one to detect when a pointed connected space is deloopable.
      Lastly, we will use universality of colimits to prove the James Splitting Theorem for general \inftytops/ and derive the reduced ordinary homology of loop spaces $\Omega S^n$ with $n\geq 2$ in $\spaces$ from it.
    \end{abstract}
    \tableofcontents
    \cleardoubleoddstandardpage
    \startpagenumbering
