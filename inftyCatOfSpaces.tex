In the following we will describe three model structures that will be used throughout: %TODO maybe better to say we will collect props/stuff about them
...
\end{definition}
\begin{definition}[Hurewicz Cofibration] %TODO maybe write a bit better, e.g. add names to maps
    ...
    \begin{center}
        \begin{tikzcd} [sep = 1.5em]
            A \arrow[r,"\id_A\times\set{0}"] \arrow[d, "i"'] &[2.5em] A\times I \arrow[d, "i\times\id_I"] \arrow[ddr, bend left] & \\[2.5em]
...
\begin{thm}[\Strom/ Model Structure on $\Top$]\label{thm:stromModelStructure}
...
\end{thm}
\begin{remark}
    Since every h-fibration is also a fibration in the Quillen model structure and every homotopy equivalence is a weak equivalence, every cofibration of the Quillen model structure is an h-cofibration. %TODO add that these classes do not agree
\end{remark}
...
\begin{thm}[Quillen Equivalence of Quillen Model Structures]\label{prop:quillenEqSSetTop}
    ...
    \begin{reference} %TODO maybe formulate ,,proof'' a little bit better
        ...
        % \begin{itemize} %TODO is this needed or does it just follow from the fact that (co)unit are we?
        ...
        % The case for the unit is proven in e.g. \cite[Theorem 3.5.4.1]{kerodon} (and originally due to \cite{milnor_unit_we}) and the case for the counit can be derived from this (see \cite[Corollary 3.5.4.2]{kerodon}).
    \end{reference}
\end{thm}
