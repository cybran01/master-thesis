...
\begin{definition}[Serre Fibration]
    A map $f\colon X\to Y$ is called a \emph{Serre fibration} if has the right lifting property against the maps $\set{D^n\xhookrightarrow{(\id_{D^n},0)}D^n\times I\mid n\in\Nat}$.
\end{definition}
\begin{definition}[Hurewicz Fibration]
    A map $f\colon X\to Y$ is called a \emph{Serre fibration} if has the right lifting property against the maps $\set{X\xhookrightarrow{(\id_{X},0)}X\times I\mid X\in\Top}$.
\end{definition}
...
\begin{thm}[Quillen Model Structure on $\Top$]
    ...
    The cofibrations are the smallest saturated class containing the maps $S^{n-1}\xhookrightarrow{}D^n$ for $n\in\Nat$ (where $S^{-1}=\emptyset$) and the trivial cofibrations are the smallest saturated class containing the maps $D^n\xhookrightarrow{(\id_{D^n},0)}D^n\times I$.
    In particular, this model structure is cofibrantly generated.
    \begin{reference}
