...
\begin{lemma}
    ...
    \begin{proof}
        By %HTT 7.2.1.14.
        this is equivalent to checking that $L(x)$ is an effective epimorphism in the ordinary category $\ho(\Disc(C))$.
...
\end{lemma}
\begin{remark}
    By definition of loop spaces and explicit computation, the \Cech/ nerve of an effective map $x\colon*\to X$ (so $X$ is a pointed connected space) is given by 
    \begin{center}
        \begin{tikzcd}
