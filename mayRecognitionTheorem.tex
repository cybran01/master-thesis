...
The presentation of loop spaces here is due to \cite{segal_cat_and_cohom}.
...
\begin{definition}
    ...
    We call $C_*$ the \emph{\inftycat/ of pointed objects}, and we call on object $X$ in $C_*$ a \emph{pointed object in $C$}.
\end{definition}
\begin{remark}
...
\begin{corollary}[May Recognition Theorem]\label{prop:mayRecognitionTheoremGroup}
    ...
    \end{proof}
\end{corollary}
\addcontentsline{toc}{subsection}{The May Recognition Theorem in $\spaces$}
...
\end{corollary}
\begin{remark}
    By definition of loop spaces and explicit computation, the \Cech/ nerve of an effective map $x\colon*\to X$ in $\Top$ (meaning $X$ is a pointed connected space) is given by
    \begin{center}
        \begin{tikzcd}
