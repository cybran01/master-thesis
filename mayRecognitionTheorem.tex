...
\begin{definition}[(Augmented) Simplicial Object]
    ...
    For any finite linearly ordered set $J$ we always have a canonical nondecreasing bijection $J\cong[n]$ for some $n\in\Nat\cup\set{-1}$.
...
\begin{prop}\label{prop:groupoidObjInToposAreEffective}
    ...
    \begin{proof}
        ...
        By \cref{prop:shiftIsColimDiam} we have $\colim\limits_{\N(\Delta)^{\op}}\overline{U}|_{\N(\Delta)^{\op}}\simeq\overline{U}_{-1} = U_0$, $\overline{U}_0=U_1$.
...
\begin{remark}
    ...
    This is because when such a group object exists, a delooping of $X$ is given by taking the colimit $A=\colim\limits_{\N\left(\Delta\right)^{\op}} U$ of this group object, which gives a pointed connected space $a\colon*\to A$ that fulfills $\Omega A\simeq X$ as pointed objects by \cref{prop:mayRecognitionTheoremGroup}.
...
\begin{corollary}
    ...
    \begin{proof}
        Since $*$ has exactly one connected component, this follows from \cref{prop:effInSpaceIffSurj}.
    \end{proof}
\end{corollary}