...
\end{definition}
\begin{definition} %TODO make better
...
\end{definition}
% \begin{lemma} %TODO is this needed?
...
\begin{remark}
    ...
    A map of topological spaces $f\colon X\to Y$ with homotopy fiber $F$ if $k$-truncated if and only if for all $x\in F$ and $n>k$ we have $\pi_n(F,x)\cong 0$.
...
\begin{prop}\label{prop:effInSpaceIffSurj}
    ...
    \end{proof}
\end{prop}
\begin{corollary} %TODO CatLR Proposition 7.8.3. 
...
% \begin{definition}
%     Let $A$ be a simplicial set and let $k\geq -1$.
%     We say that $A$ is \emph{$k$-truncated} if for all $n>k$ and $x\in A$ we have $\pi_n\left(A,x\right)=0$.
% \end{definition}
% \begin{definition}
%     Let $C$ be an \inftycat/ and let $k\geq-1$.
%     We say that an object $X\in C_0$ is \emph{$k$-truncated} if for every object $Y\in C_0$, the mapping space %TODO define mapping space
%     $\Map_C(Y,X)$ is $k$-truncated as a simplicial set.
% \end{definition}
% \begin{remark}
%     Our two definitions of $k$-truncatedness agree:
%     The observation $\Map_{\spaces}(*,X)\simeq X$ proves the first implication.
    
%     For the second implication consider a fibrant simplicial set $X$.
%     We next fix models for the pointed simplicial spheres, e.g. we choose $S^0=\Delta^0\cup\Delta^0$ with the basepoint $s$ as one of the canonical choices and for $n\geq 1$ we choose $S^n=\faktor{\Delta^n}{\partial\Delta^n}$ to be pointed by the equivalence class of $\partial\Delta^n$.
%     Then for every point $x\in X$ we get a pullback square
%     \begin{center}
%         \begin{tikzcd} [sep = 1 cm]
%             \Omega^n X \arrow [r] \arrow [d] & \Map_{\spaces}(S^n,X) \arrow [->>,d, "\ev_s"]\\
%             * \arrow [r, "x"] & X
%         \end{tikzcd}
%     \end{center}
%     where for $n\geq 1$ we take the simplicial model of the sphere $S^n=\faktor{\Delta^n}{\partial\Delta^n}$ to be pointed by the basepoint $s$ given by the equivalence class of $\partial\Delta^n$.

%     If $X$ is $k$-truncated then for all $n>k$ the space $\Omega^n X$ is contractible, which implies that $\Map_{\spaces}(S^n,X)\to X$ is a weak equivalence by the long exact sequence for fibrations.

%     Since for every simplicial set $A$ the functor $\Map_{\spaces}(A,-)$ preserves weak equivalences between fibrant objets, we know that $\Map_{\spaces}\left(A,\Map_{\spaces}(S^n, X)\right)\to\Map_{\spaces}\left(A, X\right)$ is a weak equivalence as well. %TODO define mapping space in such a way that it preserves all w.e. 
%     As $\Map_{\spaces}\left(A,\Map_{\spaces}(S^n, X)\right)\simeq\Map_{\spaces}\left(S^n,\Map_{\spaces}(A, X)\right)$, from the pullback square 
%     \begin{center}
%         \begin{tikzcd} [sep = 1 cm]
%             \Omega^n \Map_{\spaces}(A, X) \arrow [r] \arrow [d] & \Map_{\spaces}\left(S^n,\Map_{\spaces}(A, X)\right) \arrow [->>,d, "\ev_s"]\\
%             * \arrow [r, "f"] & \Map_{\spaces}(A, X)
%         \end{tikzcd}
%     \end{center}
%     as above and the long exact sequence we obtain that 
%     \begin{equation*}
%         \pi_k\left(\Omega^n \Map_{\spaces}(A, X),f\right)\cong 0
%     \end{equation*}
%     for all $f\in\Omega^n\Map_{\spaces}(A, X)$ since $\Map_{\spaces}\left(S^n,\Map_{\spaces}(A, X)\right)\to\Map_{\spaces}(A, X)$ is a weak equivalence.
%     Thus the space $\Map_{\spaces}(A,X)$ is $k$-truncated. 
% \end{remark}
...
\begin{remark}
    By definition of loop spaces and explicit computation, the \Cech/ nerve of an effective map $x\colon*\to X$ in $\Top$ (so $X$ is a pointed connected space) is given by %TODO define loop spaces
    \begin{center}
        \begin{tikzcd}
