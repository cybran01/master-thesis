...
\begin{definition}
    ...
    We say that $A$ is \emph{$k$-truncated} if for all $n>k$ and $x\in A_0$ we have $\pi_n\left(A,x\right)=0$.
\end{definition}
\begin{definition}
    Let $C$ be an \inftycat/ and let $k\geq-1$.
    We say that an object $X\in C_0$ is \emph{$k$-truncated} if for every object $Y\in C_0$, the mapping space %TODO define
    $\Map_C(Y,X)$ is $k$-truncated as a simplicial set.
\end{definition}
\begin{remark}
    Our two definitions of $k$-truncatedness agree %TODO
\end{remark}
\begin{definition}
...
\begin{lemma}
    ...
    \begin{proof}
        By %HTT 7.2.1.14.
        this is equivalent to checking that $L(x)$ is an effective epimorphism in the ordinary category $\ho(\Disc(\spaces))$. %Wwhat is L? (adjoint to incl!)
        However $\ho(\Disc(\spaces))$ is equivalent to the discrete topological spaces. %TODO why is this true?
