...
\begin{remark}
    ...
    As $\Map_{\spaces}\left(A,\Map_{\spaces}(S^n, X)\right)\cong\Map_{\spaces}\left(S^n,\Map_{\spaces}(A, X)\right)$, from the pullback square 
    \begin{center}
        \begin{tikzcd} [sep = 1 cm]
    ...
    \end{equation*}
    for arbitrary $f\in\Omega^n\Map_{\spaces}(A, X)$ since $\Map_{\spaces}\left(S^n,\Map_{\spaces}(A, X)\right)\to\Map_{\spaces}(A, X)$ is a weak equivalence.
    This implies that the space $\Map_{\spaces}(A,X)$ is $k$-truncated. 
\end{remark}
\begin{definition}
...
\begin{lemma} %TODO CatLR Proposition 7.8.3. 
