...
This phenomenon was also studied by Stasheff \cite{stasheff_hspaces} in the language of H-spaces.
...
A generalization characterizing higher loop spaces was proven in \cite{may_geometry_loop} and is called the \emph{May Recognition Theorem}.
\begin{definition}[(Augmented) Simplicial Object]
    Let $C$ be an \inftycat/. 
    We call a map $U\colon\N(\Delta)^{\op}\to C$ a \emph{simplicial object of $C$}.
    We call a map $U^+\colon\N(\Delta_+)^{\op}\to C$ an \emph{augmented simplicial object of $C$}.

    For every finite linearly ordered set $J$ we have a canonical nondecreasing bijection $J\cong[n]$ for some $n\in\Nat\cup\set{-1}$.
    Analogous to the definition of simplicial sets (\cref{def:simpSet}), given an (augmented) simplicial object $U$ we will write $U_J$ for the evaluation $U_n$ under this canonical identification. 
\end{definition}
...
\begin{definition}[Group Object]\label{def:grpObj}
    ...
    If $U_0\simeq *$ where $*$ is a terminal object of $C$, then we call $U$ a \emph{group object of $C$}.
...
\begin{definition}[\Cech/ Nerve]
    Let $C$ be an \inftycat/ with finite limits and let $\Delta^1\subset\N(\Delta_+)^{\op}$ be the inclusion corresponding to the full subcategory $\set{[0]\to[-1]}\subset\Delta_+^{\op}$.
    
    Then by \cref{prop:exKanExt} there exists a right Kan-extension functor $\CN\colon\Fun(\Delta^1,C)\to\Fun(\N(\Delta_+)^{\op},C)$ along $\Delta^1\subset\N(\Delta_+)^{\op}$.
...
\begin{corollary}\label{cor:groupoidEffectiveIffColimCechNerve}
    ...
    \begin{proof}
        This follows directly from the equivalent characterization of an effective groupoid provided by \cref{prop:grpdEffectiveIfPullback}.
    \end{proof}
\end{corollary}
...
\begin{prop}\label{prop:groupoidObjInToposAreEffective}
    ...
    \begin{proof}
        ...
        \begin{center}
            \begin{tikzcd} [sep = 4em]
                \overline{U}_n=U_{n+1} \arrow[r] \arrow[d,"{\overline{U}\left(\set{f(m)}\subset[n]\right)}"'] & U_n \arrow[d, "{U\left(\set{f(m)}\subset[n]\right)}"] \\
                \overline{U}_{\set{f(m)}}=U_{\set{f(m),n+1}} \arrow[r] & U_{\set{f(m)}}
            \end{tikzcd}
        \end{center}
        ...
        Thus the natural transformation $\overline{U}|_{\N(\Delta)^{\op}}\to U$ induced by the inclusions $[n]\xhookrightarrow{}[n+1]$ is cartesian as claimed.
        ...
        By \cref{prop:shiftIsColimDiam} we have $\colim\limits_{\N(\Delta)^{\op}}\overline{U}|_{\N(\Delta)^{\op}}\simeq\overline{U}_{-1} = U_0$, and we have $\overline{U}_0=U_1$ by construction.
        Since the map $\overline{U}_{0}\to\colim\limits_{\N(\Delta)^{\op}}\overline{U}|_{\N(\Delta)^{\op}}$ corresponds to $U(d_0^1)$ under these identifications, this proves the proposition.
    \end{proof}
\end{prop}
...
\begin{prop}\label{prop:mayRecognitionTheoremGroupoid}
    Let $C$ be an \inftytop/ and let $\Fun(\Delta^1,C)_{\text{eff}}\subset\Fun(\Delta^1,C)$ be the full subcategory spanned by the effective epimorphisms.  
    Then the functors
    \begin{equation*}
    ...
    \begin{proof}
        The functors $\colim\colon\Fun(\N(\Delta)^{\op},C)\to\Fun(\N(\Delta_+)^{\op},C)$ and $\CN\colon\Fun(\Delta^1,C)\to\Fun(\N(\Delta_+)^{\op},C)$ are both Kan-extension functors along inclusions:
        The first is a left Kan-extension functor along $\N(\Delta)^{\op}\subset\N(\Delta_+)^{\op}$, and the latter a right Kan-extension functor along $\Delta^1\subset\N(\Delta_+)^{\op}$.
        ...
        Then by \cref{prop:groupoidObjInToposAreEffective} and \cref{cor:groupoidEffectiveIffColimCechNerve} we know that it is a \Cech/ nerve.
        Since $\CN(f)|_{\Delta^1}\simeq f$ because $\CN(-)$ is a fully faithful right adjoint, it is in the essential image of $G$.
    \end{proof}
\end{prop}
The above proposition is a generalization of the delooping of pointed objects to groupoids.
We can restrict it appropriately to obtain the desired version for group objects.
...
\begin{definition}\label{def:catOfPtdConnObj}
    ...
    We call $\Conn(C_*)$ the \emph{\inftycat/ of pointed connected objects of $C$} and refer to objects of this category as \emph{pointed connected objects of $C$}.
\end{definition}
\begin{remark}
    We will prove in the next section that this notion of pointed connected objects characterizes the pointed connected spaces in $\spaces$.
\end{remark}
...
\begin{definition}[Loop Object]\label{def:loopSpace}
    ...
    Note that by the universal property there exists an essentially unique map $*\to\Omega X$, hence $\Omega X$ is canonically pointed.
\end{definition}
\begin{remark}
    ...
    This is because when such a group object $U$ exists, a delooping of $X$ is given by taking the colimit $A=\colim\limits_{\N\left(\Delta\right)^{\op}} U$ of $U$ because this is a pointed connected space $a\colon*\to A$ such that $\Omega A\simeq X$ as pointed objects by \cref{prop:mayRecognitionTheoremGroup}.
...
\begin{remark}
    ...
    Then by explicit computation and \cref{rmk:cechNerveExplicit}, one can show that the \Cech/ nerve of the map $x_0\colon*\to X$ is given by
    \begin{center}
        \begin{tikzcd}
            & \CN(x)_3 & \CN(x)_2 & \CN(x)_1 & \CN(x)_0 &  \CN(x)_{-1} \\ 
            \ldots \arrow[r] \arrow[r, yshift=-1em] \arrow[r, yshift=1em] \arrow[r, yshift=-2em] \arrow[r, yshift=2em] 
...
Thus we recover the statement of the May Recognition Theorem for $n=1$, which says that a pointed space is equivalent to a loop space of a pointed connected object if and only if it admits the structure of a group object in $\spaces$. 
\begin{definition}\label{def:truncatedMap}
    Let $C$ be an \inftytop/.
...
\begin{prop}\label{prop:inclOfSetIsFF}
    ...
    Furthermore, the functor $\pi_0\colon\spaces\to\N(\Set)$ induced by the path-connected component functor $\pi_0\colon\Top\to\Set$ is left adjoint to $i$.
    \begin{reference}
...
\begin{prop}\label{prop:testEffEpiInDisc}
    ...
    Then a map $f$ in $C$ is an effective epimorphism if and only if $\tau(f)$ is an effective epimorphism in the ordinary category $\ho(\Disc(C))$ (where we view $\tau(f)$ as a map in $\ho(\Disc(C))$ via \cref{rmk:boardmanVogt}).
    \begin{reference}
        \cite[Proposition 7.2.1.14]{HTT}
    \end{reference}
\end{prop}
\begin{remark}
    By \cite[Proposition 5.5.6.18]{HTT} a left adjoint to the inclusion $\Disc(C)\subset C$ always exists.
   
    Furthermore, it follows from the argument given in \cite[Notation 5.5.6.2]{HTT} that $\Disc(C)$ is in fact equivalent to $\N(\ho(\Disc(C)))$.
\end{remark}
\begin{prop}\label{prop:effInSpaceIffSurj}
    A map $f\colon x\to y$ in $\spaces$ is an effective epimorphism if and only if it induces a surjection on path-connected components.
    \begin{proof}
...
\begin{corollary}
    ...
    \begin{proof}
        Since $*$ has exactly one path-connected component, this follows from \cref{prop:effInSpaceIffSurj}.
    \end{proof}
\end{corollary}