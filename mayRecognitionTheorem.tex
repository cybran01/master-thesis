...
\begin{definition}[Groupoid Object]
    Let $C$ be an \inftycat/ and let $U\colon\N(\Delta)^{\op}\to C$ be a simplicial object such that for every $n\in\Nat$ and partition $[n]=S\cup T$ with $S\cap T=\set{s}$ consisting of a single element, the square (see \cref{def:simpSet})  %TODO change, this does not make sense since its not an sSet
    \begin{center}
...
\begin{definition}[Effective Groupoid Object]
    ...
    \begin{center} %TODO add/explain face maps!
        \begin{tikzcd}[sep = 4em]
