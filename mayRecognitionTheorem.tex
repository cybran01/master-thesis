...
Thus we recover the statement of the May Recognition Theorem for $n=1$, which says that a pointed space is equivalent to a loop space of a pointed connected object if and only if it admits the structure of a group object in $\Top$. %TODO do i mean a loop space of a conn obj?
\begin{definition}
    Let $C$ be an \inftytop/.
