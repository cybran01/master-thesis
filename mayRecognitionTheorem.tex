A pointed object $X$ in an \inftytop/ is \emph{deloopable} if there exists a pointed connected object $A$ such that $X\simeq\Omega A$ (as pointed objects). %TODO maybe introduce loop space here and not only in james splitting thm

In this section we will give a proof of the fact that a pointed object $X$ is deloopable if and only if it admits additional algebraic structure, which we assemble into a \emph{group object} (see \cref{def:grpObj}). %TODO mention stasheff
...
Thus we recover the statement of the May Recognition Theorem for $n=1$, which says that a pointed space is equivalent to a loop space if and only if it admits the structure of a group object in $\Top$. %TODO do i mean a loop space of a conn obj?
\begin{definition}
    Let $C$ be an \inftytop/.
...
\end{corollary}
\begin{remark}
    By the definition of loop spaces and explicit computation, the \Cech/ nerve of a map $x\colon*\to X$ in $\Top$ is given by
    \begin{center}
        \begin{tikzcd}
    ...
    \end{itemize}
    We can thus view the map $\CN\colon S_*\to\Fun(\N(\Delta_+)^{\op},\spaces)$ as being a canonically enriched version of the loop space functor. %TODO is \C a map $\spaces_*\to\Grp(\spaces)$?
    The statement of \cref{prop:mayRecognitionTheoremGroup} therefore allows us to recognize whether a pointed space $*\to X$ is deloopable:

    It is deloopable if and only if there exists a group object $U\colon\N\left(\Delta\right)^{\op}\to\spaces$ such that 
    \begin{itemize}
        \item $U_0\simeq*$
        \item $U_1\simeq X$
        \item for $c_{[1]}\colon[1]\to[0]$ the unique constant map in $\Delta$, we have that $U(c_{[1]})$ is equivalent to the map $*\to X$
    \end{itemize}
    which in particular for $n\geq1$ implies $U_n\simeq X\times\ldots\times X$ with $X$ appearing $n$ times.
    The delooping is then given by taking the colimit of this group object, which gives a pointed connected space $A\colon*\to\colim\limits_{\N\left(\Delta\right)^{\op}} U$ that fulfills $\Omega A\simeq X$.
\end{remark}