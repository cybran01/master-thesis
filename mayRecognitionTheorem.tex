...
\begin{definition}[Groupoid Object]
    Let $C$ be an \inftycat/ and let $U\colon\N(\Delta)^{\op}\to C$ be a simplicial object such that for every $n\in\Nat$ and partition $[n]=S\cup S'$ with $S\cap S'$ consisting of a single element $\set{s}$ the square
    \begin{center}
        \begin{tikzcd}[sep = 4em]
            U_n \ar[r] \ar[d] & U(S) \ar[d]\\
            U(S') \ar[r] &  U(\set{s})
        \end{tikzcd}
    \end{center}
    ...
    \begin{proof}
        ...
        \begin{center}
            \begin{tikzcd} [sep = 4em]
                ...
                \overline{U}(\set{m})=U_{\set{m,m+1}} \arrow[r] & U_{\set{m}}
            \end{tikzcd}
        \end{center}
        ...
        \begin{center}
            \begin{tikzcd} [sep = 4em]
                ...
                \overline{U}({m})=U_{\set{f(m),n+1}} \arrow[r] & U_{\set{f(n)}}
            \end{tikzcd}
        \end{center}
...
\begin{remark}
    ...
    It will follow from \cref{prop:conn} \ref{prop:connSection} that the notion of a connected pointed space $*\to X$ here means that the space $X$ is $0$-connected in the language of \cref{def:connected}, hence we call these objects connected.
\end{remark}
...
\end{definition}
\begin{remark} %TODO rewrite properly
